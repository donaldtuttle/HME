...
canon. It preserves the operational order:
    ...
    ψ input → W(t) projection → Ψmeta → Λψ? → Θλ/HME commit → QMesh lineage
...
---------
    ...
    engine.encode_memory([0.1, 0.4, 0.9, 0.2], position=(20, 22), glyph="Θλ")
    result = engine.step()
        ...
        if self._last_memory_node is not None:
            self.add_edge(
                QMeshEdge(
                    source=self._last_memory_node,
                    target=node_id,
                    relation="Θλ:memory_to_collapse",
                    weight=event.score,
                )
            )
...
def _self_test() -> dict[str, Any]:
    ...
    artifact_a = engine_a.encode_memory(payload, (12, 12), 0.4, glyph="Θλ")
    artifact_b = engine_b.encode_memory(payload, (12, 12), 0.4, glyph="Θλ")
